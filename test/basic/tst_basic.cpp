...
void test(const std::function<void(const QVariant&)> &cb){
...
void basic::xml()
{
    ...
    qDebug() << ser.serialize(QPoint{1, 2});
    qDebug() << ser.serialize(QPointF{1.2, 3.4});
    ...
    auto s  = ser.serialize(QPoint{1, 2});;
    auto ds = ser.deserialize(s, QMetaType::QPoint);
    qDebug() << ds;
}
